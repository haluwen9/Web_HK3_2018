  ...
  <div class="single-product-area">
    ...
    <div class="product-show container">
      <div class="row justify-content-center">
        ...
        <script>
          var data = <?php echo json_encode($productList) ?>;
          var filtedData = [];
          const maxLoad = 12;
            ...
            for (var i = (page - 1) * 12; i < Math.min(page * 12, array.length); i++) {
              ...
              content += `
                    <div class="col-lg-3 col-md-4 col-sm-6" style="max-width: 300px; padding: 0">
                      <div class="single-shop-product" ondragstart="itemDrag(event)" draggable="true" data-itemid="${it.id}" data-itemprice="${(1-it.sale)*it.price}">
                        <div class="product-upper">
                          <a href="?page=product&id=${it.id}" draggable="false">
                            <img src="${it.imageLink}" alt="" style="display: block; margin: 0 auto; width: 250px; height: 280px;" draggable="false">
                            <h2>${it.name}</h2>
                          </a>
                        </div>
                        <div class="product-carousel-price">
                          <ins>${formatNumber((1-it.sale)*it.price)}<sup>đ</sup></ins> <del>${formatNumber(it.price)}<sup>đ</sup></del>
                        </div>

                        <div class="product-option-shop">
                          <a href="#" class="add_to_cart_button" data-quantity="1" data-product_sku="" data-product_id="70" rel="nofollow" onclick="addToCart({id:'${it.id}',price: ${(1-it.sale)*it.price}}); return false;" draggable="false">Thêm vào giỏ hàng</a>
                        </div>
                      </div>
                    </div>`
            }
          ...
          function shopFilter(filterStr, searchStr = "") {
            ...
            filtedData = data.filter(exp => ((filterStr == "" || filterStr == 'undefined') ? true : (exp.category == filterStr))
              && (exp.category.toLocaleLowerCase().includes(searchStr) || exp.name.toLocaleLowerCase().includes(searchStr)));
